...
export default function SaveTheDateSection() {
  ...
  const cells = useMemo(
    () => buildCalendar(WEDDING_YEAR, WEDDING_MONTH),
    [],
  );
  ...
  return (
    <section className={styles.section}>
      ...
      <div className={styles.container}>
        ...
        >
          <span className={`reveal-left reveal-delay-1 ${v4 ? 'visible' : ''} ${styles.bigNum}`}>
          ...
          <span className={styles.bigDot}>·</span>
          <span className={`reveal-right reveal-delay-1 ${v4 ? 'visible' : ''} ${styles.bigNum}`}>
        ...
        >
          <div className={styles.monthLabel}>{MONTH_NAME} {WEDDING_YEAR}</div>

          <div className={styles.grid}>
            {WEEKDAYS.map((d) => (
              <span key={d} className={`${styles.cell} ${styles.weekdayHeader} ${d === 'сб' || d === 'вс' ? styles.weekend : ''}`}>
                {d}
              </span>
            ))}
            {cells.map((day, i) => {
